...
<div class="col-md-6">
            <form action="{{ route('thrifty.import') }}" method="POST" enctype="multipart/form-data" class="d-flex flex-column gap-2 shadow-sm p-3 bg-white rounded">
                @csrf
...
<a href="{{ route('scanner') }}" class="btn btn-outline-secondary btn-sm d-flex align-items-center gap-2">
    ...
    <a href="/thrifties/csv-all" target="_blank" class="btn btn-primary rounded-2">CSV</a>
    <a href="/thrifties/pdf" target="_blank" class="btn btn-primary rounded-2">Export</a>
</div>
        ...
        <div class="col-md-4 mb-4">
            <div class="card">
                <div class="card-body text-center">
                    <img src="{{ asset($thrifty->image) }}" alt="{{ $thrifty->name }}" class="img-fluid mb-2" style="max-height: 200px;">
                     {!! QrCode::size(100)->generate(Request::url() . '/thrifties/' . $thrifty->id) !!}
                    <h5 class="card-title">{{ $thrifty->brand_name }}</h5>
                    <p class="card-text">{{ $thrifty->description }}</p>
                    <p class="card-text">{{ $thrifty->price }}</p>
                </div>
